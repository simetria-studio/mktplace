...
@section('container')
        ...
        <div id="carousel_banner" class="carousel slide" data-ride="carousel">
            <ol class="carousel-indicators">
                @for ($i = 0; $i < banner_configs('home-page-principal')->count(); $i++)
                    <li data-target="#carousel_banner" data-slide-to="{{ $i }}"
            ...
            <div class="carousel-inner">
                @foreach (banner_configs('home-page-principal') as $key => $banner)
                    <div class="carousel-item @if ($key == 0) active @endif">
                        <a href="{{ $banner->link }}" @if ($banner->new_tab == 1) target="_blank" @endif><img
                                src="{{ $banner->url_file }}" class="d-block w-100" alt="{{ $banner->file_name }}"
                                title="{{ $banner->file_name }}"></a>
                    </div>
                @endforeach
            ...
            <a class="carousel-control-prev" href="#carousel_banner" role="button" data-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"><img width="40%"
                        src="{{ asset('site/imgs/previous.png') }}" alt=""></span>
                <span class="sr-only">Previous</span>
            </a>
            <a class="carousel-control-next" href="#carousel_banner" role="button" data-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"><img width="40%"
                        src="{{ asset('site/imgs/next.png') }}" alt=""></span>
        ...
        <div id="carousel_banner_mobile" class="carousel slide" data-ride="carousel">
            <ol class="carousel-indicators">
                @for ($i = 0; $i < banner_configs('home-page-principal-mobile')->count(); $i++)
                    <li data-target="#carousel_banner_mobile" data-slide-to="{{ $i }}"
                        @if ($i == 0) class="active" @endif></li>
                @endfor
            </ol>
            <div class="carousel-inner">
    ...
    <div class="custom-container my-1 mt-md-4">
                ...
                <div class="row mb-1">
                    @foreach ($produtosPereciveis as $produto)
                        @include('components.singleProduct', [
                            'produto' => $produto,
                            'class' => 'col-md-3',
                            'product_gtag_type' => 'Produtos Pereciveis',
                        ])
                    @endforeach
                </div>
            @else
        ...
        <div class="row">
            @foreach ($produtosNovos as $produto)
                @include('components.singleProduct', [
                    'produto' => $produto,
                    'class' => 'col-md-3',
                    'product_gtag_type' => 'Produtos Novos',
                ])
            @endforeach
        </div>
        ...
        {{-- <div class="my-5 d-none d-sm-block text-center" style="position: relative;">
            ...
            <a href="{{ route('seller.register') }}" target="_blank"
                style="background-color: transparent;position: absolute;height: 100%;width: 100%;left: 0;"></a>
        </div>
