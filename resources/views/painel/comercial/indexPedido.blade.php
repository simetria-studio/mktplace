                                ...
                                <div class="row">
                                    <form action="" method="GET">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <a href="{{asset('admin/comercial/pedidos')}}" class="btn btn-secondary ml-2">Limpar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
                                            ...
                                            <tr class="tr-order-id-{{$order->order_number}}">
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group" aria-label="">
                                                        <a href="{{route('ver_pedido', $order->order_number)}}" class="btn {{$order->status_v == 1 ? 'btn-dark' : 'btn-primary'}} btn-sm">Visualizar Pedido</a>
                                                        <button type="button" class="btn btn-info btn-sm alterarStatusOrder" data-url="{{route('admin.atualizar_status_order')}}" data-order_number="{{$order->order_number}}">Alterar Status</button>
                                                    </div>
                                                </td>
                                                {{-- <td>{{$order->user_name}}</td> --}}
                                                <td>
                                                    @if ($order->pay == 0)
                                                        <button type="button" class="btn btn-sm btn-block btn-status-pay" style="background-color: #fdc300;min-height: 50px;">Aguardando Pagamento</button>
                                                    @elseif ($order->pay == 1)
                                                        <button type="button" class="btn btn-sm btn-block btn-status-pay" style="background-color: #58bc9a;min-height: 50px;">Em Andamento</button>
                                                    @elseif ($order->pay == 2)
                                                        <button type="button" class="btn btn-sm btn-block btn-status-pay" style="background-color: #c6d300;min-height: 50px;">Finalizado</button>
                                                    @elseif ($order->pay == 3)
                                                        <button type="button" class="btn btn-sm btn-block btn-status-pay" style="background-color: #db5812;min-height: 50px;">Cancelado</button>
                                                    @elseif($order->pay == 10)
                                                        <div class="btn-group" role="group" aria-label="">
                                                            <button type="button" class="btn btn-sm btn-block btn-status-pay" style="background-color: #db5812;min-height: 50px;">Solicitação de Cancelamento</button>
                                                            <button type="button" class="btn btn-sm btn-info btn-verifica-cancelamento" data-toggle="modal" data-target="#verificarSolicitacaoCancelamento" data-order_number="{{$order->order_number}}" data-url="{{route('admin.verificarSolicitacaoCancelamento')}}">Verificar</button>
                                                        </div>
                                                    @elseif($order->pay == 11)
                                                ...
                                                <td>
                                                    @if ($order->shippingCustomer->tracking_id)
                                                        ...
                                                        @else
                                                            ...
                                                            @endif
                                                        @endif
                                                        <button type="button" class="btn btn-danger btn-sm btn-destroy" data-url="{{route('admin.codigo_remove')}}" data-id="{{$order->order_number}}"><i class="fas fa-times"></i></button>
                                                        @if (str_contains(verificaUrlTrackind($order->shippingCustomer->tracking_id), '<a target="_blank"'))
                                                            </div>
                                                        @endif
                                                    @else
                                                        <button type="button" class="btn btn-info btn-sm codigoAdd" data-url="{{route('admin.codigo_add')}}" data-order_number="{{$order->order_number}}">Adicionar</button>
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @if ($order->status_v == 0)
                                                        ...
                                                        <br>
                                                        <button type="button" class="btn btn-info btn-sm notificarVendedor" data-url="{{route('admin.notificarVendedor')}}" data-order_number="{{$order->order_number}}">Notificar</button>
                                                    @else
                                                        Visualizado
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    <a href="{{route('impressaoPedido', $order->order_number)}}" class="btn btn-primary btn-sm" target="_blank">Imprimir Pedido</a>
                                                </td>
                                            </tr>
                                        @empty
                                        @endforelse
    ...
    <div class="modal fade" id="verificarSolicitacaoCancelamento">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('admin.confirmarSolicitacaoCancelamento')}}" method="post">
    ...
    <script>
        $(document).ready(function(){
            ...
            $(document).on('change', '.path-anexo', function(){
                ...
                $.ajax({
                    url: `{{route('admin.orderAnexarFiscal')}}`,
                    type: 'POST',
                    data: form_data,
                    cache: false,
                    contentType: false,
            ...
            $(document).on('click', '.btn-remove-file', function(){
                ...
                $.ajax({
                    url: `{{route('admin.orderDesanexarFiscal')}}`,
                    type: 'POST',
