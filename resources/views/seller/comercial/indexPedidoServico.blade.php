                                ...
                                <div class="row">
                                    <form action="" method="GET">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <a href="{{asset('vendedor/comercial/pedidos-servicos')}}" class="btn btn-secondary ml-2">Limpar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
                                            ...
                                            <tr class="tr-order-id-{{$order->order_number}}">
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group" aria-label="">
                                                        <a href="{{route('seller.ver_pedido.servico', $order->order_number)}}" class="btn btn-primary btn-sm">Vizualizar Pedido</a>
                                                        <button type="button" class="btn btn-info btn-sm alterarStatusOrder" data-url="{{route('seller.atualizar_status_order_service')}}" data-order_number="{{$order->order_number}}">Alterar Status</button>
                                                    </div>
                                                </td>
                                                <td>
                                                    @if ($order->pay == 0)
                                                    ...
                                                    @elseif($order->pay == 10)
                                                        <div class="btn-group" role="group" aria-label="">
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-info btn-verifica-cancelamento" data-toggle="modal" data-target="#verificarSolicitacaoCancelamento" data-order_number="{{$order->order_number}}" data-url="{{route('seller.verificarSolicitacaoCancelamentoServico')}}">Verificar</button>
                                                        </div>
                                                    @elseif($order->pay == 11)
                                                ...
                                                <td>
                                                    <a href="{{route('seller.impressaoPedidoServico', $order->order_number)}}" class="btn btn-primary btn-sm" target="_blank">Imprimir Pedido</a>
                                                </td>
                                            </tr>
                                        @empty
                                        @endforelse
    ...
    <div class="modal fade" id="verificarSolicitacaoCancelamento">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('seller.confirmarSolicitacaoCancelamentoServico')}}" method="post">
    ...
    <script>
        $(document).ready(function(){
            ...
            $(document).on('change', '.path-anexo', function(){
                ...
                $.ajax({
                    url: `{{route('seller.orderServiceAnexarFiscal')}}`,
                    type: 'POST',
                    data: form_data,
                    cache: false,
                    contentType: false,
            ...
            $(document).on('click', '.btn-remove-file', function(){
                ...
                $.ajax({
                    url: `{{route('seller.orderServiceDesanexarFiscal')}}`,
                    type: 'POST',
